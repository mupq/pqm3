...
extern int crypto_hashblocks_sha512_m3_inner(unsigned char *,const unsigned char *,unsigned int,const uint64_t *);

int crypto_hashblocks_sha512(unsigned char *statebytes,const unsigned char *in,unsigned long long inlen)
{
  while (inlen >= CUTOFF) {
    crypto_hashblocks_sha512_m3_inner(statebytes,in,CUTOFF,constants); /* returns 0 */
    in += CUTOFF;
    inlen -= CUTOFF;
  }
  if (inlen < 128) return inlen;
  return crypto_hashblocks_sha512_m3_inner(statebytes,in,inlen,constants);
}
